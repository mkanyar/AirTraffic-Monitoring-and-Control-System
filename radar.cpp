...
void radar::populateEntered (){
	for (unsigned int i = 0; i < GLOBAL_AIRCRAFT_LIST.size(); i++){
			if (GLOBAL_AIRCRAFT_LIST[i]->getTime() <= GLOBAL_CLOCK){



				//hit_list.erase(hit_list.begin() + i);
				entered_list.push_back(
						new aircraft (
						GLOBAL_AIRCRAFT_LIST[i]->getID(),
						GLOBAL_AIRCRAFT_LIST[i]->getX(),
						GLOBAL_AIRCRAFT_LIST[i]->getY(),
						GLOBAL_AIRCRAFT_LIST[i]->getZ(),
						GLOBAL_AIRCRAFT_LIST[i]->getSpeedX(),
						GLOBAL_AIRCRAFT_LIST[i]->getSpeedY(),
						GLOBAL_AIRCRAFT_LIST[i]->getSpeedZ(),
						GLOBAL_AIRCRAFT_LIST[i]->getTime()
						));
			}
		}
}
//narrow down hit_list to active_list that are within airspace
void radar::populateAirspace(){
	for (unsigned int i = 0; i< entered_list.size(); i++){
			...
			airspace.push_back(
					new aircraft(
					entered_list[i]->getID(),
					entered_list[i]->getX(),
					entered_list[i]->getY(),
					entered_list[i]->getZ(),
					entered_list[i]->getSpeedX(),
					entered_list[i]->getSpeedY(),
					entered_list[i]->getSpeedZ(),
					entered_list[i]->getTime()
					));
			}
	}
}
